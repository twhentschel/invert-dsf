...
```{code-cell} ipython3
def plotsensitivity(
    q, t, d, subregion, ν_model, kwarglist, kwlabel=None, resolution=200
):
    """
    ...
    subregion: tuple
        ...
        original plot
    ν_model: callable
        the collision frequency function
    ...
    for kw in kwarglist:
        ...
        axs[1].loglog(ω, ν(ω), c=pl[-1].get_color(), label=label)
...
axs = plotsensitivity(
    q[0], t[1], d[1], [0.1, 1, 0.2, 0.6], ν_model_1, modelparams, "rise"
)
axs[1].legend()
```
...
modelparams = [{"width": 10, "rise": x} for x in [0, 0.5, 1, 1.5, 1.75, 2]]
axs = plotsensitivity(
    q[0], t[0], d[1], [0.1, 1, 0.2, 0.6], ν_model_1, modelparams
)
...
modelparams = [{"width": 10, "rise": x} for x in [0, 0.5, 1, 1.5, 1.75, 2]]
axs = plotsensitivity(
    q[0], t[1], d[2], [0.1, 3, 0.2, 2], ν_model_1, modelparams
)
...
modelparams = [{"width": 10, "rise": x} for x in [0, 0.5, 1, 1.5, 1.75, 2]]
axs = plotsensitivity(
    q[1], t[1], d[1], [0.1, 1.5, 0.1, 0.4], ν_model_1, modelparams
)
...
Before looking at the sensitivities, I'm curious to see what the imaginary part of the collision frequencies look like.
