...
plt.plot(freq_grid, normed_elf_full_data.T, lw=1, c="black", label=["TD-DFT", ""])
...
```{code-cell} ipython3
np.expand_dims(np.max(elf_full_data, axis=1), axis=1)
```
...
def elfmodel(freq, params):
    ...
    # normalize each row
    return elf / np.expand_dims(np.max(elf_full_data, axis=1), axis=1)# np.expand_dims(np.max(elf, axis=1), axis=1)
```
...
dataset = f"{residualtype} residual - q = {wavenum} - {int(percentage_of_peak * 100)}% peak threshold - TDDFT peak normalized"
```

```{code-cell} ipython3
runinfo = {
    "date" : datetime.today().strftime('%a %d %b %Y, %I:%M%p'),
    "input data info" : f"""Linear-interpolated, normalized TD-DFT data.""",
    ...
    "likelihood function" : f"Squared Exponential with a std. deviaton value of {lik_sigma}, normalized to TD-DFT data peak",
...
with h5py.File(samplesfile, "a") as f:
    # newname = 'abs residual - q = [0.78 1.55] - 80% peak threshold - TDDFT peak normalized'
    # oldname = 'abs residual - q = [0.78 1.55] - 80% peak threshold - contant normalized'
    # f[newname] = f[oldname]
    # del f[oldname]
    print(list(f.keys()))
